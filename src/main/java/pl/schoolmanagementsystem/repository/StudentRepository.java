...
public interface StudentRepository extends JpaRepository<Student, Integer> {
    ...
    @Query("select new pl.schoolmanagementsystem.model.dto.MarkAvgDto(m.subject.name, avg(m.mark)) " +
    ...
    @Query("select new pl.schoolmanagementsystem.model.dto.MarkDtoWithTwoFields(m.mark, m.subject.name) " +
            "from Student s left join Mark m on m.student=s where s.id=?1")
    List<MarkDtoWithTwoFields> findAllMarksForStudentById(int id);

    @Query("select new pl.schoolmanagementsystem.model.dto.output.StudentOutputDto2(s.id, s.name, s.surname) " +
